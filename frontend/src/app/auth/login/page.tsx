      ...
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>{t("auth_login_subtitle")}</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted-foreground hover:text-foreground" onClick={() => setShowPassword(!showPassword)}>
                    {showPassword ? t("auth_hide") : t("auth_show")}
                  </button>
                </div>
              </div>
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? <Spinner className="h-4 w-4" /> : t("auth_login_btn")}
              </Button>
            </form>

            <div className="text-center text-sm text-muted-foreground">
              <Link href="/auth/forgot-password" className="font-medium text-primary hover:underline">{t("auth_forgot")}</Link>
            </div>
            <p className="text-center text-sm text-muted-foreground">
              {t("auth_no_account")}{" "}
              <Link href="/auth/register" className="font-medium text-primary hover:underline">{t("auth_create_account")}</Link>
            </p>
          </CardContent>
        </Card>
      </div>
